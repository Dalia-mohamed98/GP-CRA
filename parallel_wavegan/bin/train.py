...
class Trainer(object):
    ...
    def _train_step(self, batch):
        ...
        #x = list of audio
        #y = list of features
        x = tuple([x_.to(self.device) for x_ in x]) # pass each audio of the list to the device then convert the list[] to a tuple()
        y = y.to(self.device) # pass features to the device

        
        """      Generator      """
        
        # 1-calculate generator loss
        y_ = self.model["generator"](*x) # pass the audios tuple after unpacking it to the generator: aud1 aud2 aud3???
        #y gtound truth signal
        #y_ generated signal
        y, y_ = y.squeeze(1), y_.squeeze(1)
        sc_loss, mag_loss = self.criterion["stft"](y_, y) 
        #compute Spectral convergence loss value and Log STFT magnitude loss value between generated and real
        gen_loss = sc_loss + mag_loss # sum for the total generator loss
        if self.steps > self.config["discriminator_train_start_steps"]:# if we started to train discriminator 
            # keep compatibility
            # The adversarial loss is defined by a continuously trained discriminator network. 
            # It is a binary classifier that differentiates between ground truth data and generated data predicted by the generative network
            gen_loss *= self.config.get("lambda_aux_after_introduce_adv_loss", 1.0) #get the coeff to multiply by gen loss
            p_ = self.model["discriminator"](y_.unsqueeze(1)) # pass the generated signal to the disc
            #p_ is the generated signal from disc.
            if not isinstance(p_, list):# for standard discriminator
                adv_loss = self.criterion["mse"](p_, p_.new_ones(p_.size())) #compute adverserial loss(mean square error) between the generated signal from disc. and ones
                #Ladv(G, D) = Ez∼N(0,I)(1 − D(G(z)))^2
                self.total_train_loss["train/adversarial_loss"] += adv_loss.item() # add to the total train adversiral loss
            else:  # for multi-scale discriminator
                adv_loss = 0.0
                for i in range(len(p_)):#element by element
                ...
                if self.config["use_feat_match_loss"]:
                    ...
                    with torch.no_grad():
                        p = self.model["discriminator"](y.unsqueeze(1)) #compute adverserial loss(mean square error) between the real signal and ones
                        #p is the real signal
                    fm_loss = 0.0
                    for i in range(len(p_)):
                        for j in range(len(p_[i]) - 1):
                            fm_loss += self.criterion["l1"](p_[i][j], p[i][j].detach())
                    fm_loss /= (i + 1) * (j + 1)
                    self.total_train_loss["train/feature_matching_loss"] += fm_loss.item()# add to the total train feature matching loss
                    adv_loss += self.config["lambda_feat_match"] * fm_loss #get the coeff to add to adv loss

            gen_loss += self.config["lambda_adv"] * adv_loss #sum the relative adv_loss val with the generator loss

        #updat total losses while training:
        ...
        # update generator
        self.optimizer["generator"].zero_grad() # run the optimizer (R_ADAM) and set gradient to 0
        ...
        gen_loss.backward()#back probagation
        # pytorch follows the graph backwards and populates the .grad member of each tensor with the partial dervative of loss with respect to that tensor using the chain rule (i.e. backpropagation)
        if self.config["generator_grad_norm"] > 0: #positive noramlized gradient 
            torch.nn.utils.clip_grad_norm_( #clips the norm of the overall gradient by concatenating all parameters passed to the function
                self.model["generator"].parameters(),
                self.config["generator_grad_norm"])
        self.optimizer["generator"].step() # performs a parameter update based on the current gradient
        self.scheduler["generator"].step() # performs learning rate(scheduler) update based on the current gradient ()
        #changing the order will skip the intial lr val then are unable to reproduce results 
        ...
        if self.steps > self.config["discriminator_train_start_steps"]:# if we started to train discriminator 
            ...
            #P gtound truth signal
            #P_ generated signal from generator after passing to disc.
            if not isinstance(p, list): # for standard discriminator
                # the discriminator is trained to correctly classify the generated sample as fake 
                # while classifying the ground truth as real using the following optimization criterion:
                #LD(G, D) = Ex∼pdata [(1 − D(x))^2] + Ez∼N(0,I)[D(G(z))^2]
                real_loss = self.criterion["mse"](p, p.new_ones(p.size()))  #mean square error of real signal
                fake_loss = self.criterion["mse"](p_, p_.new_zeros(p_.size())) # mean square error of generated signal
                #sum the losses
                ...
                real_loss = 0.0
                fake_loss = 0.0
                for i in range(len(p)):#element by element
            ...
            self.optimizer["discriminator"].zero_grad()# run the optimizer (R_ADAM) and set gradient to 0
            dis_loss.backward()#back probagation
            if self.config["discriminator_grad_norm"] > 0:#positive noramlized gradient 
                torch.nn.utils.clip_grad_norm_(#clips the norm of the overall gradient by concatenating all parameters passed to the function
                    self.model["discriminator"].parameters(),
                    self.config["discriminator_grad_norm"])
            self.optimizer["discriminator"].step()# performs a parameter update based on the current gradient
            self.scheduler["discriminator"].step()# performs learning rate(scheduler) update based on the current gradient ()
        #changing the order will skip the intial lr val then are unable to reproduce results 
    ...
    def _train_epoch(self):
        ...
        #train_steps_per_epoch : list starting from one
        #batch:audio,spectogram
        for train_steps_per_epoch, batch in enumerate(self.data_loader["train"], 1):
            ...
            """
            # check interval
            if self.config["rank"] == 0: # main process to save intermedite results
    ...
    @torch.no_grad()
    def _eval_step(self, batch): # evaluation (look at trian comments)
...
class Collater(object):
    ...
    def __init__(self,
        ...
        self.batch_max_frames = batch_max_steps // hop_size # floor divisoin
        self.hop_size = hop_size
        self.aux_context_window = aux_context_window
        self.use_noise_input = use_noise_input

    def __call__(self, batch): # Not Used (No random input)
